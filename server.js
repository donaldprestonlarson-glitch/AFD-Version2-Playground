...
let useDb = false;
let memUsers = [
  { id: 1, name: 'Dee', age: 30, city: 'Edmonton', bio: 'Owner - Actually Free!', email: 'dee@example.com', password: '$2a$10$...', photos: [], created: new Date().toISOString(), isAdmin:true },
  { id: 2, name: 'Sarah', age: 28, city: 'Edmonton', bio: 'Love hiking!', email: 'sarah.e@example.com', password: '$2a$10$...', photos: [], created: new Date().toISOString() }
];
let memMessages = [];
let nextId = 3;
...
app.get('/api/users', async (req,res)=>{
  ...
  if(useDb){
    try{
      ...
      // extra safety sort for Dee
      list.sort((a,b)=>{
        const aDee = a.name && a.name.toLowerCase()==='dee' || a.isAdmin;
        const bDee = b.name && b.name.toLowerCase()==='dee' || b.isAdmin;
        if(aDee && !bDee) return -1;
        if(!aDee && bDee) return 1;
        return 0;
      });
  ...
  res.json(list.map(safeUser).reverse().sort((a,b)=>{
    const aDee = a.name.toLowerCase()==='dee';
    const bDee = b.name.toLowerCase()==='dee';
    if(aDee && !bDee) return -1;
    if(!aDee && bDee) return 1;
    return 0;
  }));
...
app.get('/api/me', async (req,res)=>{
  ...
  try{
    ...
    const u = memUsers.find(x=>x.id===d.id);
...
app.post('/api/signup', upload.array('photos',4), async (req,res)=>{
  try{
    ...
    if(useDb){
      ...
      const r = await pool.query('INSERT INTO users (name,email,password,age,city,bio,photos) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *',
        [name,email,hashed,parseInt(age)||25,city||'Edmonton',bio||'',JSON.stringify(photoUrls)]);
      const u = r.rows[0];
    ...
    if(memUsers.find(u=>u.email.toLowerCase()===email.toLowerCase())) return res.status(400).json({error:'Email already used'});
...
app.post('/api/login', async (req,res)=>{
  ...
  if(useDb){
    ...
    u = memUsers.find(x=>x.email.toLowerCase()===email.toLowerCase());
  }
...
app.post('/api/update-profile', upload.array('photos',4), async (req,res)=>{
  try{
    ...
    let keep = [];
    try{ keep = keepPhotos ? JSON.parse(keepPhotos) : []; }catch{ keep=[]; }
    ...
    if(useDb){
      ...
      const cur = r.rows[0];
      let curPhotos = [];
      try{ curPhotos = typeof cur.photos==='string'? JSON.parse(cur.photos) : cur.photos||[]; }catch{ curPhotos=[]; }
      if(keep.length===0) keep=curPhotos;
      let merged = [...keep, ...newUrls].slice(0,4);
      await pool.query('UPDATE users SET name=COALESCE($1,name), age=COALESCE($2,age), city=COALESCE($3,city), bio=COALESCE($4,bio), photos=$5 WHERE id=$6',
        [name||null, age?parseInt(age):null, city||null, bio, JSON.stringify(merged), d.id]);
      ...
      const u = memUsers.find(x=>x.id===d.id);
      if(!u) return res.status(404).json({error:'Not found'});
      if(name) u.name=name; if(age) u.age=parseInt(age); if(city) u.city=city; if(bio!==undefined) u.bio=bio;
      if(keep.length===0) keep=u.photos||[];
      u.photos=[...keep,...newUrls].slice(0,4);
      return res.json({message:'Updated!', photos:u.photos, ...safeUser(u)});
    }
...
app.get('/api/messages/:myId', async (req,res)=>{
  ...
  const filtered = memMessages.filter(m=> (m.from_id===myId && m.to_id===withId) || (m.from_id===withId && m.to_id===myId));
...
app.post('/api/messages', async (req,res)=>{
  ...
  const msg = { id: memMessages.length+1, from_id, to_id, text, at: new Date().toISOString() };
...
app.get('/api/admin/users', checkAdmin, async (req,res)=>{
  ...
  res.json(memUsers.map(u=>({ ...safeUser(u), email: u.email, totalPhotos: u.photos?.length||0 })).reverse());
});
app.post('/api/admin/delete/:id', checkAdmin, async (req,res)=>{
  ...
  if(useDb){
    ...
    memMessages = memMessages.filter(m=>m.from_id!==id && m.to_id!==id);
  }
...
app.post('/api/admin/remove-photo', checkAdmin, async (req,res)=>{
  ...
  if(useDb){
    ...
    let photos = [];
    try{ photos = typeof r.rows[0].photos==='string'?JSON.parse(r.rows[0].photos):r.rows[0].photos||[]; }catch{ photos=[]; }
  ...
  const u = memUsers.find(x=>x.id===parseInt(userId));
  if(!u) return res.status(404).json({error:'User not found'});
  u.photos = (u.photos||[]).filter(p=>p!==photoUrl);
  res.json({message:'Photo removed', photos: u.photos});
});
app.get('/admin', (req,res)=> res.sendFile(__dirname+'/public/admin.html'));
app.listen(PORT, ()=> console.log(`AFD FREE+DB ready on ${PORT} - useDb=${useDb} - Admin pass set: ${ADMIN_PASS!== 'AFD-Admin-2026!'}`));
