...
// --- Simple in-memory DB ---
let users = [
  { id: 1, name: 'Sarah', age: 28, city: 'Edmonton', bio: 'Love hiking in River Valley! Looking for someone real — no BS. Actually free is refreshing!', email: 'sarah.e@example.com', password: '$2a$10$demo', photos: [], created: new Date().toISOString() },
  { id: 2, name: 'Mike', age: 32, city: 'Calgary', bio: 'Calgary born, love Flames and mountains. Let\'s grab coffee — my treat, messaging is free!', email: 'mike.c@example.com', password: '$2a$10$demo', photos: [], created: new Date().toISOString() }
];
let messages = [];
let nextId = 3;
...
app.post('/api/update-profile', upload.array('photos',4), async (req,res)=>{
  try{
    ...
    let keep = [];
    if(keepPhotos){
      try{ keep = JSON.parse(keepPhotos); }catch{ keep = u.photos || []; }
    } else {
      keep = u.photos || [];
    }
...
app.listen(PORT, ()=> console.log(`AFD Live on ${PORT} - Ready for 4 photos! Admin pass: ${ADMIN_PASS}`));
